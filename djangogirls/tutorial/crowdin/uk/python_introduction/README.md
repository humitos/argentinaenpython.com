...
Поки що, ми не хочемо залишати консоль Python. Ми хочемо вивчити її краще. Давайте почнемо з чогось справді простого. Наприклад, спробуйте ввести якісь математичні вирази, на зразок `2 + 3` і натисніть `Enter`.
...
Для того, щоб видалити що-небудь із списку, вам потрібно використовувати вище вивчені **індекси** і команду **del** (del є абревіатурою від delete). Давайте спробуємо на прикладі та нагадаємо, що ми вивчили раніше; ми будемо видаляти перший елемент з нашого списку.
...
Цікаво чому ми використали два однакові знаки `==` щоб порівняти чи є рівними числа? Ми використовуємо `=` для присвоєння значень змінним. Завжди, **завжди** треба писати `==` якщо ви бажаєте здійснити перевірку двох чисел на рівність одне одному. Ми також можемо перевіряти чи два об'єкта є нерівними. Для цього ми використовуємо символ `!=`, як показано в прикладі вище.
...
Раніше, ми вже обрали для себе редактор коду, в секції [редактор коду][2]. А тепер ми маємо відкрити цей редактор і написати якийсь код в новому файлі:
...
А тепер треба зберегти файл і дати йому описове ім'я. Назвімо файл **python_intro.py** і збережемо його на вашому робочому столі. Ви можете називати файл як завгодно, тільки важливо щоб назва закінчувалась на **.py**. Закінчення **.py** говорить нашій операційної системи, що це **виконуваний файл Python** і Python можне запускати його.
