...
> ถ้าคุณยังเจอข้อผิดพลาด `TemplateDoesNotExists` ให้ลองปิดและเปิดเซิร์ฟเวอร์อีกครั้ง โดย เปิดบรรทัดคำสั่งขึ้น ปิดเซิร์ฟเวอร์โดยใช้คีย์ Ctrl+C (ปุ่ม Control และ C พร้อมกัน) และเริ่มอีกครั้งโดยใช้คำสั่ง `python manage.py runserver`
...
*   tag พื้นฐาน `<html>` เป็นจุดเริ่มต้นของทุกหน้าเว็บ และ `</html>` อยู่ล่างสุดเสมอ อย่างที่คุณเห็น เนื้อหาทั้งหมดของเว็บไซต์ เกิดขึ้นระหว่าง tag เริ่มต้น `<html>` และปิดท้ายด้วย tag `</html>`
...
*   `<h1>A heading</h1>` - สำหรับ หัวเรื่องที่สำคัญที่สุด
...
*   อีกสอง `div` คือส่วนที่แสดงเนื้อหาบล็อกและวันที่เผยแพร่ โดยใช้ `h2` สำหรับหัวข้อโพสต์ และใช้สอง `p` (ย่อหน้า) สำหรับวันที่และสำหรับบล็อกโพสต์ของเรา
...
*   เปิด [หน้าคอนโซล PythonAnywhere][5] และไปยัง **Bash console** ของคุณ (หรือเริ่มอันใหม่) จากนั้นใช้คำสั่ง:
...
*   สุดท้าย ไปยัง [Web tab][6] และกดปุ่ม **Reload** บนเว็บของคุณ
