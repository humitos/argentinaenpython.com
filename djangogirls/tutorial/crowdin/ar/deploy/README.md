...
حتى الآن كان موقع الويب الخاص بك متوفر فقط على جهاز الكمبيوتر الخاص بك، والآن سوف تتعلم كيفية اعداده! الإعداد هو عملية نشر التطبيق الخاص بك على شبكة الإنترنت حيث يمكن للناس الذهاب إلى التطبيق الخاص بك و مشاهدته :).

كما تعلمت، موقع على شبكة الإنترنت يجب أن يتواجد في خادم. هناك الكثير من مقدمي خدمات استضافة المواقع على مخدمات متوفرون على شبكة الإنترنت. We will use one that has a relatively simple deployment process: [PythonAnywhere][1]. PythonAnywhere متوفر مجانا للتطبيقات الصغيرة التي لا تحتوي على عدد كبير من الزائرين فالآن بالتأكيد ستكون كافية بالنسبة لك.

 [1]: http://pythonanywhere.com/

The other external service we'll be using is [GitHub][2], which is a code hosting service. هناك آخرون ، ولكن تقريبا جميع المبرمجين لديهم حساب GitHub في هذه الأيام، والآن أنت سوف يمكنك أيضا الحصول على حساب!

 [2]: http://www.github.com

سوف نستخدم GitHub كنقطة انطلاق لنقل كودنا (التعليمات البرمجية) من و إلى PythonAnywhere.
...
> **ملاحظة** إذا كنت قمت بإجراء خطوات التثبيت سابقا، لا تحتاج للقيام بذلك مرة أخرى -يمكنك تخطي إلى الجزء التالي والبدء في إنشاء مستودع Git الخاص بك.

{% include "deploy/install_git.md" %}

## ابتداء مستودعك في Git

Git يتعقب التغييرات التي تحدث لمجموعة معينة من الملفات في ما يسمى بمستودع الكود (أو "الريبو" للاختصار). لنبدأ انشاء مستودع (ريبو) لمشروعنا. Open up your console and run these commands, in the `djangogirls` directory:
...
تهيئة المستودع هو شيء نحتاج القيام به مرة واحدة فقط لأي مشروع (ولن تضطر إلى إعادة إدخال اسم المستخدم والبريد الإلكتروني مرة أخرى).

Git سوف يتعقب التغييرات لكافة الملفات والمجلدات الموجودة في هذا الدليل، ولكن هناك بعض الملفات التي نريد Git أن يتجاهلها. We do this by creating a file called `.gitignore` in the base directory. افتح المحرر الخاص بك وقم بإنشاء ملف جديد مع محتويات التالية:
...
واحفظه ك `.gitignore` في المجلد العلوي "djangogirls".

> **ملاحظة** النقطه في بداية اسم الملف مهمه! إذا كنت تواجه أي صعوبة في انشائه (على سبيل المثال ،أجهزة ماكينتوش لا تمكنك من إنشاء الملفات التي تبدأ بنقطة عن طريق الباحث)، ثم استخدم ميزة "حفظ باسم" في المحرر الخاص بك.
