...
Υπάρχει μια έννοια στον προγραμματισμό που ονομάζεται `"Αντικειμενοστρεφής Προγραμματισμός"`. Η ιδέα είναι ότι αντί να γράφουμε τα πάντα ως μία βαρετή ακολουθία από εντολές προγραμματισμού, μπορούμε να μοντελοποιήσουμε πράγματα και να καθορίσουμε το πώς αλληλεπιδρούν μεταξύ τους.
...
Αν θέλουμε να μοντελοποιήσουμε μια γάτα, θα δημιουργήσουμε ένα αντικείμενο `γάτα` που έχει ορισμένες ιδιότητες, όπως: `χρώμα`, `ηλικία`, `διάθεση` (δηλαδή καλή, κακή, νυσταγμένη;)), και ένα αντικείμενο `ιδιοκτήτης` (που είναι ένα `άτομο` ως αντικείμενο ή ίσως, στη περίπτωση μιας αδέσποτης γάτας, η ιδιότητα αυτή είναι κενή).

Τότε η `γάτα` έχει κάποιες συμπεριφορές: `γουργουρίζει`, `γδέρνει` ή `τρέφεται` (στην οποία θα δώσουμε στη σγάτα κάποια `Τροφή (CatFood)`, που θα μπορούσε να είναι ένα ξεχωριστό αντικείμενο με ιδιότητες, δηλαδή `γεύση`).
...
Λοιπόν, σίγουρα μία ανάρτηση στο blog μας χρειάζεται κάποιο κείμενο με το περιεχόμενό του και έναν τίτλο, σωστά; Θα ήταν επίσης ωραίο να ξέρουμε ποιος το έγραψε - έτσι χρειαζόμαστε έναν συγγραφέα. Τέλος, θέλουμε να γνωρίζουμε πότε δημιουργήθηκε και δημοσιεύτηκε η συγκεκριμένη ανάρτηση.
